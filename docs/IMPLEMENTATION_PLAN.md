...
| 1.4.15 | Domain: `AddOnGroup` + `AddOnItem` entities (separate, reusable) + `MenuItemAddOnLink` junction            | domain       | 1.4.2        | NOT_STARTED | AddOnGroup(id, tenantId, name, items, sortOrder, isActive). AddOnItem(id, groupId, name, price, maxQty, inventoryItemId?, sortOrder, isActive). Junction: MenuItemAddOnLink(menuItemId, addOnGroupId, sortOrder). Qty-based, selalu ada harga |
| 1.4.16 | Domain: VOs (`AddOnGroupId`, `AddOnItemId`)                                                                | domain       | 1.1.7        | NOT_STARTED | ULID-based value classes                                                                                                                                                                                                                  |
| 1.4.17 | Domain: Repository interface `AddOnGroupRepository`                                                        | domain       | 1.4.15       | NOT_STARTED | CRUD group+items, link CRUD, listByTenant, getLinksForItem                                                                                                                                                                                |
| 1.4.18 | Domain: Use cases (SaveAddOnGroup, GetAddOnGroups, DeleteAddOnGroup)                                       | domain       | 1.4.17       | NOT_STARTED | 3 use cases: CRUD add-on group with items                                                                                                                                                                                                 |
| 1.4.19 | Data: Room entities + DAOs + mappers (AddOnGroup, AddOnItem, MenuItemAddOnGroup)                           | data         | 1.4.17,1.1.4 | NOT_STARTED | addon_groups, addon_items, menu_item_addon_groups tables. Sync metadata on all. FK CASCADE on items                                                                                                                                       |
| 1.4.20 | Data: `AddOnGroupRepositoryImpl`                                                                           | data         | 1.4.19       | NOT_STARTED | group + items + links CRUD, same pattern as ModifierGroupRepositoryImpl                                                                                                                                                                   |
| 1.4.21 | UI: Add-on group management screen                                                                         | presentation | 1.4.18       | NOT_STARTED | AddOnGroupManagementScreen (list with inline items, price, maxQty) + AddOnGroupFormScreen (dynamic items list). CatalogMainScreen: add "Add-on" nav tab                                                                                   |
| 1.4.22 | Domain: `SelectedAddOn` VO + OrderLine.addOns + addOnTotal() + updated lineTotal()                         | domain       | 1.4.15,1.5.2 | NOT_STARTED | SelectedAddOn(addOnName, quantity, unitPrice, totalPrice). lineTotal = (effectiveUnitPrice * qty) + addOnTotal - discount. addOnTotal NOT multiplied by OrderLine qty                                                                      |
| 1.4.23 | Data: OrderLineEntity.addOnSnapshot (JSON) + serialization                                                 | data         | 1.4.22       | NOT_STARTED | Same snapshot pattern as modifierSnapshot. JSON: `[{n:"Extra Cheese",q:2,u:5000,t:10000}]`                                                                                                                                               |
| 1.4.24 | UI: Add-on selection in POS (during add-to-cart)                                                           | presentation | 1.4.22       | NOT_STARTED | AddOnSelectionContent: qty stepper per add-on item, running total, max qty validation. Shown after modifier selection in bottom sheet                                                                                                     |
| 1.4.25 | Unit tests: AddOnGroup domain, use cases, SelectedAddOn + lineTotal                                        | test         | 1.4.15-22    | NOT_STARTED | Add-on model tests, use case tests with fake repo, OrderLine price calc with add-ons                                                                                                                                                     |
...
| 2.1.10 | UI: Add-on selection during add-to-cart                                                                                                 | presentation | 1.4.24     | NOT_STARTED | AddOnSelectionContent: qty stepper per add-on item, max qty enforcement, running total. Shown after modifier selection. Items with add-ons always new line |
| 2.1.11 | UI: Add-on display in cart, receipt, kitchen ticket                                                                                     | presentation | 2.1.10     | NOT_STARTED | Cart: show add-on per line (name × qty @price). Receipt: add-on lines indented under menu item. Kitchen ticket: include add-on info |
